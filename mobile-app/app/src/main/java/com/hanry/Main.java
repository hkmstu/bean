...
public class Main extends Activity implements SeekBar.OnSeekBarChangeListener
{
    ...
    private String CAMERA_VIDEO_URL = "http://192.168.31.137:8080/?action=stream";
    private String CAMERA_VIDEO_URL_TEST = "";
    private String ROUTER_CONTROL_URL = "192.168.31.137";
    private String ROUTER_CONTROL_URL_TEST = "192.168.31.137";
    private int ROUTER_CONTROL_PORT = 2001;
    private int ROUTER_CONTROL_PORT_TEST = 2001;
    private final String WIFI_SSID_PERFIX = "ice";
...
//        buttonLen.setKeepScreenOn(true);
    ...
    private OnLongClickListener buttonCus1ClickListener2 = new OnLongClickListener() {
        public boolean onLongClick(View arg0) {
            ...
            try {
                ...
                mLogText.setText("connect failed!" +  e.getMessage());
            }
    ...
    private void sendCommand(byte[] data) {
        if ( mWifiStatus != STATUS_CONNECTED || null == mtcpSocket) {
            mLogText.setText("wifi connect failed" +  data.toString());
            return;
        }
    ...
    private void handleCallback(byte[] command) {
        ...
        switch (cmd2) {
        case (byte)0x01:
            mLogText.setText("case (byte)0x01");
        	handleHeartBreak();
        	break;
        case (byte)0x02:
            handleHeartBreak();
    ...
    private void connectToRouter(boolean isTest) {
        ...
        if (WIFI_STATE_CONNECTED == status || isTest) {
            ...
            mLogText.setText("wifi not connected");
        } else {
            mLogText.setText("wifi status unrecognized");
        }
    }
    ...
    protected void onResume() {
        ...
        if (WIFI_STATE_CONNECTED == status || m4test) {
            ...
            if (null != cameraUrl && cameraUrl.length() > 4) {
            	backgroundView.setSource(cameraUrl);//��ʼ��Camera
            }
        }
        super.onResume();
    }
    ...
    public void onBackPressed() {
        if (mQuitFlag) {
            ...
            mQuitFlag = true;
            Toast.makeText(mContext, "what", Toast.LENGTH_LONG).show();
            Message msg = new Message();    
